...
class PlayerManager:
    ...
    def init_players(self):
        for player in self.manager.props.player_names:
            ...
            if self.selected_player is not None and self.selected_player != player.name:
                logger.debug(f"{player.name} is not in the filtered player, skipping it")
                continue
    ...
    def on_player_appeared(self, _, player):
        ...
        if player.name in self.excluded_player:
            logger.debug("New player is in exclusion list, skipping it")
            return
...
def parse_arguments():
    ...
    parser.add_argument("-x", "--exclude", "- Comma-separated list of excluded players")
...
def main():
    ...
    if arguments.enable_logging:
        logfile = os.path.join(os.path.dirname(os.path.realpath(__file__)), "mediaplayer.log")
    ...
    if arguments.exclude:
        logger.info(f"Excluding player {arguments.exclude}")
